...
/* para compilar no linux: gcc golfinho3.c -lglfw -lGL -lGLEW -lm -o golfinho3.bin */
...
typedef struct{
    ...
    float wcamada1[2 + 2*(pop_peixe)][4][pop_golfinho];
    float wcamada2[4][7][pop_golfinho];

static void rede_neural( int pop, float resposta[7], int golfinho, float pesocamada1[2 + 2*(pop_peixe)][4][pop], float pesocamada2[4][7][pop], float vel_final,float direc_finalUP, float direc_finalDOWN, float direc_finalRIGHT, float direc_finalLEFT, float rede_final,float fome_final,float distXpeixe[pop_peixe],float distYpeixe[pop_peixe]){
    ...
    for(int j=0 ; j< 4;j++){
        for(int k=0; k<pop_peixe;k++){
            intermediario[j]= intermediario[j] + distXpeixe[k]*pesocamada1[2 + k][j][golfinho] + distYpeixe[k]*pesocamada1[2 + pop_peixe+k][j][golfinho];
        }
    ...
    for(int j = 0;j<4 ;j++){
        ...
        intermediario[j]= intermediario[j] + vel_final*pesocamada1[0][j][golfinho] + fome_final*pesocamada1[1][j][golfinho]  ;
...
static void nova_geracao(){
    ...
    int ind_melhor=0;
    float peso_melhor[2 + 2*(pop_peixe)][4];
    float peso2_melhor[4][6];
    int gene_mutado_i;
    int gene_mutado_j;
    int ad_sub_mutado;
    int matrizpeso_mutada;
    int total_genes1 = 2 + 2*(pop_peixe) * 4;
       ...
       // srand(time(NULL)); srand null aqui faz com q todos os individuos tenham o msmo valor d chance d mutacao e gene mutado
         ...
         if(k != ind_melhor){
            ...
            if(matrizpeso_mutada<=50){//MUTAR O PESO1
                gene_mutado_i = rand()%(2 + 2*(pop_peixe));
          ...
          //  printf("\n\n\n\nGOLFIN %d\n\n",i );
              ...
              //  printf("Melhor %d Score do %d: %f\n",melhor,i,tobias[i].score );
               ...
               // printf("vida do %d:%f\n",i,tobias[i].vida );
                ...
                // if( vetor_transicao[6]>0.4 ){ //sigo golfin em x
                //     if((pacu[0].posY - tobias[i].posY) < 0){// peixe na esquerda
                //         tobias[i].posY = tobias[i].posY - vetor_transicao[0]*vel_global;
                //     }else{
                //         tobias[i].posY = tobias[i].posY + vetor_transicao[0]*vel_global;
                //     }
                ...
                if(tobias[i].direcRIGHT>0.4 ){ //sigo golfin em x
                    tobias[i].direcRIGHT=0;
                ...
                if(tobias[i].direcLEFT<0.4){
                 ...
                 if(tobias[i].direcUP<0.4){// sigo o golfin em y
                    tobias[i].direcUP=0;
                }
                if(tobias[i].direcDOWN<0.4){
                    tobias[i].direcDOWN=0;
                ...
                if( tobias[i].veloc_atual<0.4){
                 ...
                 if( vetor_transicao[5]>0.9 ){ //sigo golfin em x
                    if((pacu[0].posX - tobias[i].posX) < 0){// peixe na esquerda
                        tobias[i].posX = tobias[i].posX - vetor_transicao[0]*vel_global;
                    }else{
                        tobias[i].posX = tobias[i].posX + vetor_transicao[0]*vel_global;
                    }
